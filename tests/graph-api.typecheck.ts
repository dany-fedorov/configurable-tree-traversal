...
const promisedTreeVisitor: AsyncTraversalVisitor<'ORDER', Input, Output> =
  async () => undefined;
...
const dagVisitor: DagVisitor<Input, Output> = (vertex, options) => {
  ...
  return {
    commands: [
      {
        commandName: TraversalVisitorCommandName.REWRITE_VERTEX_HINTS_ON_PRE_ORDER,
...
const ref = new CTTRef(
  new Vertex<Input>({ $d: { input: 'vertex' }, $c: [] }),
);
void [
  asyncGraph,
  asyncTree,
